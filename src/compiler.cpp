...
public:
    ...
    std::string generateCode(const std::string& assembly) {
        ...
        // Check platform and architecture
            ...
            // ARM64 macOS assembly (Apple Silicon)
            code += "section .text\n";
            code += "global _main\n";
            code += "extern _printf\n\n";
            ...
            // Convert x86 assembly to ARM64
            // Instead of the full assembly converted (which would be complex),
            // we'll just hard-code our example for demonstration:
            code += "    mov x0, #5\n";               // Set value to 5
            code += "    mov x1, x0\n";               // Copy to x1 for square
            code += "    mul x0, x0, x1\n";           // Square the value
            code += "    str x0, [x28, #0]\n";        // Store at variable x
            
            // Print result
            code += "    adrp x0, fmt@PAGE\n";
            code += "    add x0, x0, fmt@PAGEOFF\n";
            code += "    mov x1, x0\n";               // Pass result as argument
            code += "    bl _printf\n";
            ...
            code += "section .data\n";
            code += "fmt: .asciz \"Result: %lld\\n\"\n";
        #else
            // x86 macOS assembly (Intel Mac)
            code += "section .text\n";
            code += "global _main\n";
            code += "extern _printf\n\n";
            ...
            code += "    push rbp\n";
            code += "    mov rbp, rsp\n";
            code += assembly;  // Note: we'd need to adapt this for 64-bit
            
            // macOS stack alignment (must be 16-byte aligned before call)
            code += "    sub rsp, 8\n";         // Align stack
            code += "    mov edi, eax\n";       // First arg in edi (eax has result)
            code += "    lea rsi, [rel fmt]\n"; // Format string in rsi
            code += "    xor eax, eax\n";       // Zero eax (no vector regs)
            code += "    call _printf\n";       // Call printf
